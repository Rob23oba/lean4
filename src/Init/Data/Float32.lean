...
-/
@[extern "lean_float32_of_bits"] def Float32.ofBits (bits : UInt32) : Float32 :=
...
-/
@[extern "lean_float32_to_bits"] def Float32.toBits (x : Float32) : UInt32 :=
  ⟨x.val.encode⟩

theorem Float32.toBits_inj {x y : Float32} : x.toBits = y.toBits ↔ x = y := by
  simp only [toBits, UInt32.ofBitVec.injEq]
  constructor
  · intro h
    replace h := congrArg Std.BinaryFloat.decode h
    rw [Std.BinaryFloat.decode_encode _ rfl (by decide)] at h h
    exact congrArg Float32.mk h
  · rintro rfl; rfl

instance : DecidableEq Float32 := fun _ _ =>
  decidable_of_decidable_of_iff Float32.toBits_inj
...
@[extern "lean_uint8_to_float32"] def UInt8.toFloat32 (n : UInt8) : Float32 :=
  ⟨.ofNat n.toNat⟩

/-- Obtains the `Float32` whose value is the same as the given `UInt16`. -/
@[extern "lean_uint16_to_float32"] def UInt16.toFloat32 (n : UInt16) : Float32 :=
  ⟨.ofNat n.toNat⟩
...
-/
@[extern "lean_uint32_to_float32"] def UInt32.toFloat32 (n : UInt32) : Float32 :=
  ⟨.ofNat n.toNat⟩
...
This function is overridden at runtime with an efficient implementation.
-/
@[extern "lean_uint64_to_float32"] def UInt64.toFloat32 (n : UInt64) : Float32 :=
  ⟨.ofNat n.toNat⟩
...
This function is overridden at runtime with an efficient implementation.
-/
@[extern "lean_usize_to_float32"] def USize.toFloat32 (n : USize) : Float32 :=
  ⟨.ofNat n.toNat⟩
