...
/--
Equality of booleans. The preferred notation for this is `a == b`.

The corresponding propositional connective is `Iff : Prop → Prop → Prop`, written with the `↔`
operator.

This function offers the definitional equalities `(a == true) = a` and `(a == false) = !a`
-/
...
class Decidable (p : Prop) where
  ...
  cond_decide (p) : cond decide p (Not p)

/-- Proves that `p` is decidable by supplying a proof of `¬p` -/
@[match_pattern, inline]
abbrev Decidable.isFalse {p : Prop} (h : Not p) : Decidable p where
  decide := false
  cond_decide := h

/-- Proves that `p` is decidable by supplying a proof of `p` -/
@[match_pattern, inline]
abbrev Decidable.isTrue {p : Prop} (h : p) : Decidable p where
  decide := true
  cond_decide := h

/-- Prove something about `Decidable p` by proving it for `isFalse` and `isTrue` -/
@[cases_eliminator, induction_eliminator]
def Decidable.falseTrueCasesOn {p : Prop} {motive : Decidable p → Sort u}
    (t : Decidable p)
    (isFalse : (h : Not p) → motive (Decidable.isFalse h))
    (isTrue : (h : p) → motive (Decidable.isTrue h)) :
    motive t :=
  t.casesOn (fun d hd => d.casesOn isFalse isTrue hd)

export Decidable (isTrue isFalse decide)
open Decidable (cond_decide)
...
@[macro_inline] def dite {α : Sort u} (c : Prop) [h : Decidable c] (t : c → α) (e : Not c → α) : α :=
  h.decide.casesOn e t h.cond_decide
...
@[macro_inline] instance {p q} [dp : Decidable p] [dq : Decidable q] : Decidable (And p q) where
  ...
  cond_decide :=
    ...
    | isTrue  hp =>
      match dq with
      | isTrue hq  => ⟨hp, hq⟩
      | isFalse hq => fun h => hq (And.right h)
    | isFalse hp =>
      fun h => hp (And.left h)
...
theorem Nat.not_le_of_ble_eq_false (h : Eq (Nat.ble n m) false) : Not (LE.le n m) :=
  fun h' => Bool.noConfusion (Nat.ble_eq_true_of_le h' ▸ h :)
...
instance Nat.decLe (n m : @& Nat) : Decidable (LE.le n m) where
  decide := Nat.ble n m
  cond_decide :=
    match h : Nat.ble n m with
    | true => Nat.le_of_ble_eq_true h
    | false => Nat.not_le_of_ble_eq_false h
...
instance (n : Nat) : DecidableEq (Fin n) :=
  fun i j => {
    ...
    cond_decide :=
      match decide (Eq i.val j.val), cond_decide (Eq i.val j.val) with
      | true, h  => Fin.eq_of_val_eq h
      | false, h => fun h' => absurd (Fin.val_eq_of_eq h') h
  }
...
instance : DecidableEq Char :=
  fun c d => {
    ...
    cond_decide :=
      match decide (Eq c.val d.val), cond_decide (Eq c.val d.val) with
      | true, h  => Char.eq_of_val_eq h
      | false, h => Char.ne_of_val_ne h
  }
...
Normally used via the == operator.
-/
...
protected def List.hasDecEq {α : Type u} [DecidableEq α] (a b : List α) : Decidable (Eq a b) where
  decide := a.beq b
  cond_decide := go a b
where
  go : (a b : List α) → cond (a.beq b) (Eq a b) (Not (Eq a b))
  | nil,       nil       => rfl
  | cons _ _, nil        => fun h => List.noConfusion h
  | nil,       cons _ _  => fun h => List.noConfusion h
  | cons a as, cons b bs =>
    show cond (and (decide (Eq a b)) (List.beq as bs)) _ _ from
    match decide (Eq a b), cond_decide (Eq a b) with
    | true, hab  =>
      match List.beq as bs, go as bs with
      | true, habs  => hab ▸ habs ▸ rfl
      | false, nabs => fun h => List.noConfusion h (fun _ habs => absurd habs nabs)
    | false, nab => fun h => List.noConfusion h (fun hab _ => absurd hab nab)
...
instance : DecidableEq String.Pos :=
  fun ⟨a⟩ ⟨b⟩ => {
    decide := decide (Eq a b)
    cond_decide :=
      match decide (Eq a b), cond_decide (Eq a b) with
      | true, h  => h ▸ rfl
      | false, h => fun he => String.Pos.noConfusion he fun he => absurd he h
  }
